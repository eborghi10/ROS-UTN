...
# TODO: Use rosparam for some constants and dynamic reconfigure
    ...
    def __init__(self, kP, kI, kD, motor_max_vel=10, target_velocity=0, rate_hz=30, i_clamp=10, alpha=1.0, \
        encoder_topic='encoder', motor_topic='output', set_point_topic='set_point', error_topic='error', output_vel_topic='velocity'):
        # PID parameters
        self.kP = float(kP)
        self.kI = float(kI)
        self.kD = float(kD)
        ...
        self.output_vel_pub = rospy.Publisher(output_vel_topic, Float32, queue_size=10, latch=True)
    ...
    def __get_error__(self):
        ...
        self.error_pub.publish(current_error)
        # Proportional
    ...
    def calculate(self):
        error_vel = self.__get_error__()
        self.output_vel_pub.publish(error_vel)
        self.publish_velocity(error_vel)
